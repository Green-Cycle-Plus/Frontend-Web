...
const Page = () => {
	...
	const fetchData = async () => {
		...
		try {
			...
			const offerData = await getRecyclerOffers(id);
			await getWasteTypes();
	...
	const getWasteTypes = async () => {
		try {
			const res = await axios.get(`https://api-greencycle.onrender.com/api/waste-type`, {
				headers: {
					"x-api-key": process.env.NEXT_PUBLIC_BACKEND_API_KEY,
				},
			});
			console.log("waste types", res);
							...
							<div className="grid gap-4 py-4">
								...
								<div className="flex w-full items-center gap-4">
									<div className="grid gap-4 w-full">
										...
										<Select onValueChange={(value) => setValue("waste_type", value)}>
										...
										{errors.waste_type && <p className="text-red-500">{errors.waste_type.message}</p>}
									</div>
					...
					offers.map((offer) => (
						<Card>
							...
							<CardContent>
								...
								<p>
									<span className="text-[#757575]">Quantity: </span> {Number(offer.minQuantity.toString())}kg
								</p>
							</CardContent>
							...
							<CardFooter className="flex w-full justify-between">
								<p className="text-[#757575]">
									Waste type:
									<Button className="ml-2 rounded-full bg-[#228B22]">{offer.name}</Button>
								</p>
								<Button
									variant={"outline"}
